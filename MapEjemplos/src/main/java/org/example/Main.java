...
public class Main {
    public static void main(String[] args) {
        ...
        List<Integer> listaCochePrecios = new ArrayList<>();
        listaCochePrecios.add(5000);
        listaCochePrecios.add(4000);
        listaCochePrecios.add(3000);
        mapaCochePrecios.put("mini", List.of(5000,4000,3000));
        mapaCochePrecios.put("mercedes", List.of(200000,30000,50000, 1000000));
        ...
        Set<String> conjuntoClaves = mapaCochePrecios.keySet();
        /*while(conjuntoClavesIterator.hasNext()){
            String clave = conjuntoClavesIterator.next();
            List<Integer> coches = mapaCochePrecios.get(clave);
            for (int i = 0; i < coches.size(); i++) {
                System.out.println(clave + "---"+ coches.get(i));
            }
        }*/
        for (String clave : conjuntoClaves) {
            List<Integer> coches = mapaCochePrecios.get(clave);
            for (Integer coch : coches) {
                System.out.println(clave + "---" + coch);
            }
        }
       ...
       // mapa.compute(5,(k,v)->v.concat("++"));
        ...
        //1. Clásica
        Iterator<Integer> it2 = listaClaves.iterator();
        while(it2.hasNext()){
            Integer aux = it2.next();
        ...
        mapa.keySet().forEach(c-> System.out.println("Clave:"+ c + "="+ " Valor: "+ mapa.get(c)));
        System.out.println("---------------Sólo muestra aquellas claves que cumplen un filtrado-----------------");
        mapa.keySet().stream().filter(k->k>=20&&k<30).forEach(k-> System.out.println("Clave:"+ k + "="+ " Valor: "+ mapa.get(k)));
